...
For a real-world example, we use the public Pima diabetes data distributed with `MASS`, linked to the classification problem studied by Smith and colleagues @smith1988. The figure will show observed diabetes prevalence across bins of plasma glucose and body mass index. This is a transparent partial application rather than a literal recreation of a figure in the original paper. The paper is the scientific source of the dataset and prediction problem, while the heat map is a teaching visualization built on the public data.
